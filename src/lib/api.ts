...
export const signUpMutation = async (data: SigningType) => await API.post("/auth/register", data);

export const signInMutation = async (data: SigningType) => await API.post("/auth/login", data);
...
export const editInventoryMutation = async (data: InventoryUpdatePayload): Promise<InventoryResponse> => { 
  const {id, ...payload} = data
